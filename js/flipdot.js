...
//flipdot.js is used to handle the communication with the flipdot display, converting bit arrays to hex strings and sending commands to update the display. It also includes functions for rasterizing a canvas image into a format suitable for the flipdot panels.
const NUM_COLS = 28;
const NUM_ROWS = 14;
...
const PANEL_ADDRS = ['02', '01'];// index 0 = bottom, index 1 = top
 ...
 * @param {boolean} immediate boolean, if true the panel will update as soon as the command is received
 * 
...
function process_and_send_signal() {
    let hexStr0 = bit_arr_to_hex_str(bottomPanelBits);
    let hexStr1 = bit_arr_to_hex_str(topPanelBits);

    let command0 = hex_str_to_command(hexStr0, 0, false); // bottom → address 0x02
    let command1 = hex_str_to_command(hexStr1, 1, false); // top    → address 0x01
...
/**
 * Sends the current canvas image to the flipdot display
 * by rasterizing pixel brightness and updating each panel.
 */

function extractPanelBits(brightnesses) {
    const top = [], bottom = [];
    for (let x = 0; x < NUM_COLS; x++) {
      for (let y = 0; y < NUM_ROWS; y++) {
        const i = x + y * NUM_COLS;
        const bit = brightnesses[i];
        if (y < 7) top.push(bit);
        else bottom.push(bit);
      }
    }
    return [top, bottom];
  }
...
function canvas2display() {
    ...
    for (let x = 0; x < NUM_COLS; x++) {
        for (let y = 0; y < NUM_ROWS; y++) {
            ...
            if (y < 7) {
                topPanelBits.push(bit);
            } else {
                bottomPanelBits.push(bit);
            }
        }
    }
